...
import { createCustomer, findCustomerDuplicates, listCustomers, parseDemandPhrase, validateCustomerInput } from '../dist/api/customers.js';
...
test('parses quick demand phrase without AI', () => {
  assert.deepEqual(parseDemandPhrase('80平方米 超市李总 电话：15955555555 内蒙古'), {
    demandText: '80平方米 超市李总 电话：15955555555 内蒙古',
    storeArea: '80㎡',
    storeType: '超市',
    phone: '15955555555',
  });
  assert.equal(parseDemandPhrase('80平 便利店货架').storeArea, '80㎡');
});
...
test('detects duplicates only inside current team', () => {
  ...
  assert.deepEqual(duplicate[0].reasons, ['手机号相同', '微信相同', '地址相似']);
  assert.equal(otherTeamDuplicate.length, 0);
});


test('blocks weak customer records without useful follow-up signal', () => {
  assert.equal(validateCustomerInput(input({ name: '李总', phone: '', wechat: '', address: '', demandText: '' })), '客户信息不足，请至少填写手机号、微信、地址或明确需求。');
  assert.equal(validateCustomerInput(input({ name: '李总', phone: '15955555555', wechat: '', address: '', demandText: '' })), undefined);
});
